...
export function useColorPresets() {
  const [colorPresets, setColorPresets] = useAtom(colorPresetsAtomWithPersistence);
  return {
...
export function useColorPresetName() {
  const [colorPresetName, setColorPresetName] = useAtom(colorPresetNameAtomWithPersistence);
  return {
    colorPresetName: colorPresetName === null ? DEFAULT_PRESET_COLOR_NAME : colorPresetName,
    setColorPresetName,
  };
}

// apply color preset
export function useApplyColorPreset<T extends Record<string, any>>(colorPresets: T) {
  ...
  useEffect(() => {
    ...
    updateThemeColor(colorLighter, colorLight, colorDefault, colorDark, colorForeground);
